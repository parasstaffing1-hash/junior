...
def build_project(
    ...
    *,
    ...
    if output_path:
        ...
        files = {"html": str(html_file), "pdf": str(pdf_file), "xlsx": str(xlsx_file)}

    validation = {
        "valid": bool(layout_validation["valid"] and len(kpis) >= 2 and len(charts) >= 2 and len(tables) >= 1 and len(pdf_bytes) > 100 and len(xlsx_bytes) > 100),
        "layout_valid": bool(layout_validation["valid"]),
        "kpi_count": len(kpis),
        "chart_count": len(charts),
        "table_count": len(tables),
        "artifact_sizes": {"html_bytes": len(html_report.encode("utf-8")), "pdf_bytes": len(pdf_bytes), "xlsx_bytes": len(xlsx_bytes)},
    }
