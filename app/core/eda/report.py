...
def generate_eda_report(df:pd.DataFrame,*,title="Automated EDA Report",sections=None,numeric_columns=None,categorical_columns=None,max_group_comparisons=10,max_groups=20,max_findings=50,histogram_bins=20,relationship_sample_limit=5000,random_state=42):
    ...
    if relationship_sample_limit<100:raise EDAReportError("INVALID_SAMPLE_LIMIT","relationship_sample_limit must be >=100.")
    ...
    relationship_df=(
        df.sample(n=relationship_sample_limit,random_state=random_state).sort_index()
        if len(df)>relationship_sample_limit else df
    )
    warnings=[];out={}
    ...
    if "categorical_eda" in wanted and cats:out["categorical_eda"]=_safe("categorical_eda",lambda:analyze_categorical_eda(df,columns=cats,include_numeric_low_cardinality=False,association_cardinality_limit=max_groups,association_sample_limit=relationship_sample_limit,random_state=random_state),warnings)
    if "numeric_numeric" in wanted and len(nums)>=2:out["numeric_numeric"]=_safe("numeric_numeric",lambda:analyze_numeric_relationships(relationship_df,columns=nums),warnings)
    if "numeric_categorical" in wanted and nums and cats:out["numeric_categorical"]=_safe("numeric_categorical",lambda:analyze_numeric_categorical_relationships(relationship_df,numeric_columns=nums,categorical_columns=cats,max_groups=max_groups),warnings)
    safe_cats=_eligible_categorical(df,max_groups)
    if "categorical_categorical" in wanted and len(safe_cats)>=2:out["categorical_categorical"]=_safe("categorical_categorical",lambda:analyze_categorical_relationships(relationship_df,columns=safe_cats,include_numeric_low_cardinality=False,max_cardinality=max_groups,max_cells=max_groups*max_groups),warnings)
    if "correlation_explorer" in wanted and len(nums)>=2:out["correlation_explorer"]=_safe("correlation_explorer",lambda:explore_correlations(relationship_df,columns=nums),warnings)
    ...
    if "group_comparisons" in wanted and nums and cats and max_group_comparisons:
        ...
        for n,c,_ in sorted(candidates,key=lambda x:-x[2]):
            ...
            r=_safe(f"group_comparison:{n}:{c}",lambda n=n,c=c:analyze_group_comparison(relationship_df,value_column=n,group_column=c,max_groups=max_groups),warnings)
            if r:group_results.append({"status":"ok",**r})
        out["group_comparisons"]=group_results
    md=_markdown(title,df,out,warnings)
    return {
        "title":title,"row_count":len(df),"column_count":len(df.columns),
        "numeric_columns":nums,"categorical_columns":cats,"included_sections":wanted,
        "analysis_basis":{
            "full_row_count":len(df),"full_row_sections":["numeric_eda","categorical_profiles","distribution_explorer","findings"],
            "relationship_rows":len(relationship_df),"relationship_sampled":len(relationship_df)<len(df),
            "relationship_sample_limit":relationship_sample_limit,"random_state":random_state,
            "eligible_categorical_relationship_columns":safe_cats,
            "skipped_high_cardinality_relationship_columns":[c for c in cats if c not in safe_cats],
        },
        "sections":out,"warnings":warnings,"report_markdown":md,"report_html":_html(title,md)
    }
