...
CAPABILITIES: tuple[dict[str, Any], ...] = (
    ...
    {"id": "excel_mis_automation", "domain": "business_intelligence", "name": "Governed Excel/MIS automation", "status": "partial", "priority": "high", "evidence": "Source-backed XLSX controls, formulas, Power Query plan, reconciliation, exceptions, lineage-aware manual/automatic MIS plans and audit records; native Excel Desktop, signed VBA/.xlsm, PivotTable/slicer refresh and Microsoft 365 delivery remain approval-gated integrations"},
    ...
    {"id": "subscriptions_alerts", "domain": "collaboration", "name": "Subscriptions, threshold alerts and delivery", "status": "available", "priority": "high", "evidence": "Tenant-scoped alert rules, deterministic evaluation, scheduled metric discovery, audited webhook delivery, approval gating, and durable schedules"},
    ...
    {"id": "privacy_compliance", "domain": "governance", "name": "PII discovery, retention and compliance controls", "status": "available", "priority": "critical", "evidence": "PII discovery, masking preview, tenant-scoped retention policies, dry-run evaluation, approval-gated deletion, legal holds, and immutable retention-run evidence"},
    ...
    {"id": "observability", "domain": "platform", "name": "SLOs, tracing, cost and usage observability", "status": "partial", "priority": "critical", "evidence": "Request IDs, audit logs, bounded route p50/p95/p99 SLO evidence, Prometheus metrics, health checks, job state, storage telemetry, and cost metadata exist; tracing backend, capacity telemetry, and customer SLO policy remain deployment-specific"},
    ...
    {"id": "ha_scalability", "domain": "platform", "name": "High availability and horizontal scalability", "status": "partial", "priority": "critical", "evidence": "Reviewable Kubernetes topology with two API replicas, two workers, probes, HPA, resource budgets and disruption protection; target-cluster, managed database/storage and load-test evidence remain deployment-specific"},
