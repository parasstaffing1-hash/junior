...
CAPABILITIES: tuple[dict[str, Any], ...] = (
    ...
    {"id": "warehouse_connectors", "domain": "data_foundation", "name": "Cloud warehouse and database connectors", "status": "planned", "priority": "critical", "evidence": "No governed Snowflake, BigQuery, Redshift, Databricks or live PostgreSQL connector service"},
    {"id": "incremental_ingestion", "domain": "data_foundation", "name": "Incremental, CDC and streaming ingestion", "status": "planned", "priority": "high", "evidence": "Current imports are batch snapshots"},
    ...
    {"id": "schema_drift", "domain": "data_foundation", "name": "Schema drift and contract enforcement", "status": "partial", "priority": "high", "evidence": "Versioned schemas exist; proactive contract monitoring is not yet scheduled"},
    ...
    {"id": "pipeline_orchestration", "domain": "analytics_engineering", "name": "Reusable pipelines and orchestration", "status": "partial", "priority": "critical", "evidence": "Allowlisted automation exists; distributed scheduling and retries remain limited"},
    {"id": "semantic_layer", "domain": "semantic_modeling", "name": "Governed semantic models and metrics", "status": "partial", "priority": "critical", "evidence": "Workspace assets persist models/KPIs; query-time metric compilation remains to be built"},
    ...
    {"id": "row_level_security", "domain": "semantic_modeling", "name": "Row and object-level security", "status": "planned", "priority": "critical", "evidence": "No policy evaluation layer"},
    ...
    {"id": "forecasting_ml", "domain": "analysis", "name": "Forecasting, ML and anomaly detection", "status": "planned", "priority": "high", "evidence": "Not part of the Tools 1-100 runtime"},
    ...
    {"id": "catalog_lineage", "domain": "governance", "name": "Searchable catalog and column-level lineage", "status": "partial", "priority": "critical", "evidence": "Dataset/version/artifact lineage exists; enterprise catalog search and column lineage are incomplete"},
    {"id": "rbac_sso", "domain": "governance", "name": "RBAC, SSO, SCIM and tenant isolation", "status": "planned", "priority": "critical", "evidence": "No identity or authorization service"},
    {"id": "privacy_compliance", "domain": "governance", "name": "PII discovery, retention and compliance controls", "status": "planned", "priority": "critical", "evidence": "No classification or retention policy engine"},
    ...
    {"id": "scheduling_workers", "domain": "platform", "name": "Distributed workers, queues and schedules", "status": "planned", "priority": "critical", "evidence": "Synchronous local execution remains the primary runtime"},
    {"id": "observability", "domain": "platform", "name": "SLOs, tracing, cost and usage observability", "status": "planned", "priority": "critical", "evidence": "Health checks exist without production telemetry or SLO enforcement"},
