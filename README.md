...
| Excel/MIS automation contract | `GET /api/v1/mis/catalog`; `POST /api/v1/datasets/{dataset_id}/mis/automation-plan`; generated workbooks include `MIS Automation`, lineage, controls, manual/automatic mode and external delivery gates |
| External BI/M365 integration gates | `GET /api/v1/integrations/catalog`; `POST /api/v1/integrations/plan`; `POST /api/v1/integrations/powerbi/validate`; Excel Desktop, SharePoint, OneDrive and Outlook side effects remain approval-gated |
...
- **Excel / MIS automation:** every source-backed XLSX includes `Raw Data`, `Pivot Summary`, `Formula Lab`, `Power Query`, `Dashboard`, `Exceptions`, `Reconciliation`, `MIS Control`, `MIS Automation`, and `Workbook Guide` sheets. The `MIS Automation` contract records source lineage, controls, manual/automatic mode, delivery targets and approval gates. Multi-sheet workbooks can be combined with `_source_sheet` lineage. Reconciliation provides row-count and numeric control totals; Exceptions provides duplicate/missing-value review; MIS Control provides an operator checklist. Formulas recalculate when opened in modern Excel. The M script is reviewable and ready for Power Query; native PivotTable/slicer refresh, VBA/.xlsm execution, Outlook email and SharePoint/OneDrive delivery require an explicitly approved external Excel/Microsoft Graph runner.
- **Power BI:** the PBIP semantic model records its fact-table grain, dimensions, relationships, measure definitions, date logic, RLS publication gate, and performance decisions in `model_contract.json` and `MODEL_DESIGN.md`; `advanced_semantic_model.json` additionally records the validated senior modeling contract. Client identities are never invented; the included RLS role fails closed until an approved mapping is configured.
