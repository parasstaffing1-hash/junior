...
7. Added reusable dashboard templates (`executive`, `sales_performance`, `ecommerce_conversion`, `operations`, and `root_cause`), template-aware BI report generation, template selection APIs, table widgets, and frontend template controls.
8. Added regression tests for templates, storage containment/upload limits, supported ingestion formats, migrations, and formula safety.
9. Removed corrupt NUL bytes from the frontend stylesheet tail and confirmed JavaScript syntax.
10. Added a catalog-driven project workbench for all 20 projects listed on the [Appwars Technologies reference page](https://appwarstechnologies.com/data-analytics-projects/), with deterministic fixtures, field aliases, domain derivations (RFM, funnel rates, forecasting baseline, inventory value/stockout risk, volatility, and more), template-backed dashboards, and HTML/PDF/XLSX artifacts.
11. Added project catalog/build/validation/artifact APIs and a frontend project lab with individual build actions plus a one-click all-20 validation action. Fixed the drag-and-drop file input selector so page initialization works with the supported connector inputs.

## Executed validation evidence

| Check | Actual result |
|---|---|
| Full regression | `python -m pytest -q`: **45 passed, 0 failed, 0 skipped** in 28.81s. Five warnings: one Starlette/httpx deprecation and four SciPy Anderson-test future warnings. |
...
The validation runner built **20/20** project packages from deterministic 48-row fixtures. Every package passed the same checks: source-backed field mapping, at least three KPIs, at least two charts, at least one detail table, collision-free dashboard-template layout, HTML rendering, PDF generation, and XLSX generation. Generated submission artifacts are checked into `examples/project_portfolio/`, with aggregate evidence in `examples/project_portfolio/validation_results.json`.

| API | Result |
|---|---|
| `GET /api/v1/project-catalog` | 20 catalogued projects |
| `POST /api/v1/projects/{project_id}/build` | Individual project build plus HTML/PDF/XLSX artifact links |
| `POST /api/v1/project-validation/run` | `COMPLETED`, 20 passed, 0 failed |
| `GET /api/v1/projects/{project_id}/artifacts/{kind}` | Generated HTML, PDF, and XLSX downloads |
...
| Inventory dashboard | PASS WITH LIMITATION | The supply-chain project derives inventory value, stockout risk, sell-through, and an operations template from the reference fixture; the legacy dashboard endpoint remains single-dataset and has no persisted inventory semantic model. |
...
| Dashboard layout | PASS | Five template layouts were generated and validated collision-free in API and live tests. |
